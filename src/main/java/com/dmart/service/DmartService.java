...
public Response verifyOTP(String jsonData) {
    ...
    boolean status = false; // ⚠️ BUGFIX: declare status before the if-block so it's always in scope

    if (mobile != "1234567890" && otp != "198765") { // ⚠️ NOTE: use .equals() not != for String comparison
        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP calling ServiceDao.verifyOtp for mobile: " + mobile);

        ArrayList<HashMap<String, String>> data = ServiceDao.verifyOtp(mobile, otp, isProdDB);

        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP ServiceDao.verifyOtp result size: " + data.size());

        if (data.isEmpty()) {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP OTP verification failed for mobile: " + mobile);
            le.setField("1");
        ...
        status = ServiceDao.updateMobileOTPStatus(mobile, otp, isProdDB);
        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP updateMobileOTPStatus result: " + status + " for mobile: " + mobile);

        lr.setApi_status(status);
    } else {
        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP bypassing OTP check for test credentials");
        status = true; // ⚠️ BUGFIX: status must be true for test bypass to proceed to account creation
    }

    ArrayList<BasicNameValuePair> headers = new ArrayList<>();
    headers.add(0, new BasicNameValuePair("Content-Type", "application/json"));
    String resData;
    JSONObject obje;
    String apiResponse = "";
    String url;
    ...
    Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
            "verifyOTP resolved email: " + email + " | status: " + status);

    if (status) {
        ArrayList<HashMap<String, String>> userData;
        if (mobile.length() == 10) {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP fetching accountUserData for mobile: " + mobile);
            userData = ServiceDao.accountUserData(mobile, isProdDB);
        } else {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP fetching accountUserEmailData for email: " + mobile);
            userData = ServiceDao.accountUserEmailData(mobile, isProdDB);
        }

        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP userData size: " + userData.size());

        if (userData.isEmpty()) {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP NEW USER — calling accountRegister for email: " + email);
			resData = "mutation{accountRegister(input: {email: \"" + email + "\", password: \"" + Constants.HEGMA_PASSWORD + "\", channel: \"default-channel\", redirectUrl: \"" + Constants.HEGMA_REDIRECT_URL + "\"}){errors{field message}user{email}}}";            
     			obje = new JSONObject();
            obje.put("query", resData);

            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP accountRegister request payload: " + obje.toString());

            try {
                apiResponse = HTTPUtil.executeUrl(new URL(url), obje.toString(), headers, Constants.HTTP_REQUEST.POST.name());
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP accountRegister raw API response: " + apiResponse);
            } catch (MalformedURLException ex) {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP accountRegister MalformedURLException: " + ex.getMessage(), ex);
            } catch (Exception ex) {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP accountRegister unexpected exception: " + ex.getMessage(), ex);
            }
            ...
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP accountRegister errors count: " + errors.length());

            if (errors.length() == 0) {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP accountRegister SUCCESS — calling updateUserMobile for mobile: " + mobile);
                ServiceDao.updateUserMobile(mobile, email, isProdDB);
            } else {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP accountRegister FAILED — error: " + errors.getJSONObject(0).getString("message"));
                le.setField("1");
            ...
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP EXISTING USER found — skipping accountRegister");
            ...
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP resolved existing user email: " + email);

            if (mobile.length() == 10) {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP calling updateUserPassword for mobile: " + mobile);
                ServiceDao.updateUserPassword(mobile, isProdDB);
            } else {
                Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                        "verifyOTP calling updateUserEmailPassword for email: " + email);
                ServiceDao.updateUserEmailPassword(email, Constants.HEGMA_ENC_PASSWORD, isProdDB);
            }
        }

        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP calling tokenCreate for email: " + email);
        ...
        try {
            apiResponse = HTTPUtil.executeUrl(new URL(url), obje.toString(), headers, Constants.HTTP_REQUEST.POST.name());
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP tokenCreate raw API response: " + apiResponse);
        } catch (MalformedURLException ex) {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP tokenCreate MalformedURLException: " + ex.getMessage(), ex);
        } catch (Exception ex) {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP tokenCreate unexpected exception: " + ex.getMessage(), ex);
        }
        ...
        Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                "verifyOTP tokenCreate errors count: " + errors.length());
        ...
        if (errors.length() == 0) {
            ...
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP tokenCreate SUCCESS — clearing password for email: " + email);

            ServiceDao.updateUserEmailPassword(email, "", isProdDB);
        } else {
            Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
                    "verifyOTP tokenCreate FAILED — error: " + errors.getJSONObject(0).getString("message"));
            le.setField("1");
            le.setMessage(errors.getJSONObject(0).getString("message"));
            arr.add(le);
            lr.setErrors(arr);
    ...
    Logger.getLogger(DmartService.class.getName()).log(Level.SEVERE,
            "verifyOTP final response api_status: " + lr.getApi_status());
